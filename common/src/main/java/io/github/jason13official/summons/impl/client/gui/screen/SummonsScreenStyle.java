...
/// Shared chrome for the Summons screens, built entirely from vanilla textures/APIs so the
/// screens read as part of the game rather than a mod overlay: the same tiled panel
/// background + separators the in-world pause menu list uses, and real hover [Tooltip]s.
final class SummonsScreenStyle {
  ...
  /// top offset for a `totalHeight`-tall content block: centered when it fits, otherwise
  /// pinned so its bottom stays an 8px margin above the screen edge (never past it) rather
  /// than centering blindly and pushing the bottom row of buttons off-screen
  ...
  /// the vanilla in-world list panel: tiled noise texture between a light header separator
  /// and a dark footer separator, same as the pause menu's button list
  ...
  /// wraps `text` to `width` and returns the panel height [#textBox] would use, so callers
  /// can lay out whatever comes after it without duplicating the wrap math
  ...
  /// word-wrapped flavor/description text inside a panel; height must come from
  /// [#textBoxHeight] (call it during layout, not here, so button positions can't drift)
