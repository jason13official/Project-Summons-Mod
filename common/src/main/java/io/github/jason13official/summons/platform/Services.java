...
public class Services {
  ...
  private static <T> T load(Class<T> clazz) {
    ...
    // A merged multi-loader jar carries every platform's provider entry, so ServiceLoader
    // can hand back one whose backing classes aren't on this platform's classpath. Touch a
    // real platform call per candidate - the wrong one throws, and we skip to the next.
